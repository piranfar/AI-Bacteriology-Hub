...
// API key lives in the browser session only — never committed to source code
const getApiKey = () => sessionStorage.getItem('gemini_api_key') ?? '';
const saveApiKey = (key: string) => sessionStorage.setItem('gemini_api_key', key);
...
function ApiKeyBanner() {
  ...
  const [visible, setVisible] = React.useState(!getApiKey());
  ...
  if (!visible) {
    return (
      <div className="bg-slate-950 border-b border-slate-800 px-4 py-2 flex items-center justify-between text-xs">
        <span className="text-green-400 font-semibold">✓ Gemini API key set</span>
        <button onClick={() => setVisible(true)} className="text-slate-500 hover:text-slate-300 underline ml-4">Change</button>
      </div>
    );
  }

  return (
    <div className="bg-amber-950/40 border-b border-amber-800/50 px-4 py-3 flex flex-col sm:flex-row items-start sm:items-center gap-3">
      <div className="flex-1 min-w-0">
        <p className="text-amber-300 text-xs font-bold uppercase tracking-wider mb-1">Gemini API Key Required</p>
        <p className="text-slate-400 text-xs">
          Get a free key at{' '}
          <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" className="text-blue-400 underline hover:text-blue-300">
            aistudio.google.com/apikey
          </a>
          . Your key is stored only in this browser session — never committed to code.
        </p>
      </div>
      <div className="flex gap-2 w-full sm:w-auto shrink-0">
        <input
          ...
          className="flex-1 sm:w-64 bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-amber-500"
        />
        <button
          onClick={handleSave}
          disabled={!key.trim()}
          className="bg-amber-600 hover:bg-amber-500 disabled:opacity-40 text-white font-bold text-sm px-4 py-1.5 rounded-lg transition whitespace-nowrap"
        >
          Save Key
        </button>
      </div>
...
// =========================================
const executeGeminiCall = async (prompt: string, systemPrompt: string, useSearch = false) => {
  const apiKey = getApiKey();
  if (!apiKey) throw new Error('No API key set. Please enter your Gemini API key in the banner above.');
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 font-sans flex flex-col">
      ...
      <header className="bg-slate-950 border-b border-slate-800 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="text-xs text-slate-400 text-right hidden sm:block">
            <div className="font-semibold text-slate-300">Principal Investigator: Vahhab Piranfar</div>
            <div>AI-Bacteriology Hub • 2026 Research Cycle</div>
          </div>
        </div>
      </header>
      ...
      {/* Footer */}
      <footer className="bg-slate-950 border-t border-slate-800 py-4 text-center mt-auto shrink-0">
        <p className="text-xs text-slate-500">
          This application developed with the help of React, Gemini API, and Python.
        </p>
      </footer>
    </div>
  );
